...
%% Load video
% video_file =  VideoReader('64caf10.avi'); % candle
video_file =  VideoReader('56ub310.avi'); % water
% video_file =  VideoReader('649j210.avi'); % checkboard
% video_file =  VideoReader('64ac220.avi'); % coral reef
...
pixel_vectors = zeros(number_of_signals, length(extracted_pixels));

index = 1;
for row=1:rows
    for column=1:rows
        pixel_vectors(index,:) = extracted_pixels(row, column,:);
        index = index+1;
    end
end
...
%% 1-3: Generate spike train from signal and simulate the spike train with 
% the same average fring rate
...
for k = 1:number_of_signals
    [spike_count, spike_index] = get_spikes(pixel_vectors(k,:),threshold, spike_interval);
...
%% Compute symetric matrix
C_ij = zeros(number_of_signals);
T = size(pixel_vectors,2);
% interval_length = 20;
% subs = zeros(T,1);

used_signal = pixel_vectors;
...
%         r_i_r_j = mean(ccg_rr); Remoed for now, because do not know what
%         is the equivalence for non spike train signal
        % correlation matrix
